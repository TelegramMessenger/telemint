...
export const storeText = (text: string) =>{
    return (builder: Builder) => {
        builder.storeUint(text.length, 8)
        builder.storeBuffer(Buffer.from(text))
    }
}

export const storeTextRef = (text: string) => {
    return (builder: Builder) => {
        builder.storeRef(beginCell().store(storeText(text)).endCell());
    }
}
